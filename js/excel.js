...
// ==========================================================
// STOCK GUDANG (Export + Import/Tambah Harga Barang)
// Stok Awal, Total Masuk, Total Keluar, Stok Akhir tetap
// dihitung otomatis. Yang bisa diinput manual/import di sini
// hanya HARGA BARANG per kode (dicocokkan dengan Data Barang).
// ==========================================================
...
async function importStockExcel(file) {
    bacaExcel(file, {
        ...
        headerBaris: ["kode", "harga barang", "nama barang"]
    },async function (rows) {
        let jumlahUpdate = 0;
        let dilewati = 0;
        rows.forEach(row => {
            const kode = String(ambilNilai(row, "KODE", "kode")).trim();
            const harga = Number(ambilNilai(row, "HARGA BARANG", "harga")) || 0;
            if (kode === "") {
                dilewati++;
                return;
            }
        ...
        if (jumlahUpdate === 0) {
            gagal("Tidak ada harga yang diperbarui. " + dilewati + " baris dilewati (kode kosong atau tidak ditemukan di Data Barang).");
        } else if (dilewati > 0) {
            sukses(jumlahUpdate + " harga barang berhasil diimport (" + dilewati + " baris dilewati).");
        } else {
            sukses(jumlahUpdate + " harga barang berhasil diimport.");
        }
    });
}
// ==========================================================
